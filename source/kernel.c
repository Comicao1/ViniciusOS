...
#define READ_U32(ptr) \
    ({ uint32_t __val = (uint32_t)((ptr)[0] | ((ptr)[1] << 8) | ((ptr)[2] << 16) | ((ptr)[3] << 24)); \
       ptr += 4; __val; })
#define READ_F32(ptr) \
...
void draw_line(unsigned char* tela, int width, int height, int x0, int y0, int x1, int y1, uint32_t color);
...
void clear_buffer(unsigned char color) {
    ...
    uint32_t* buffer32 = (uint32_t*)back_buffer;
